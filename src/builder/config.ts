...
export interface IBundleConfig
  extends FastscConfig,
    Omit<FastscBundleConfig, 'entry' | 'output'> {
  type: BuildTypes.BUNDLE;
  bundler: 'webpack';
  entry: string;
  output: {
    filename: string;
...
export interface IBundlessConfig
  extends FastscBaseConfig,
    Omit<FastscBundlessConfig, 'input' | 'overrides'> {
  type: BuildTypes.BUNDLESS;
  format: BundlessTypes;
  input: string;
}
 ...
 * @param pkg
 */
export function normalizeUserConfig(userConfig: FastscConfig, pkg: PkgType) {
  ...
  Object.entries({
    ...
    const defaultPlatform =
      formatName === 'esm' ? PlatformTypes.BROWSER : PlatformTypes.NODE;
    const bundlessConfig: Omit<IBundlessConfig, 'input'> = {
      type: BuildTypes.BUNDLESS,
      format: formatName as BundlessTypes,
...
export function createConfigProviders(
  ...
  const { bundle, bundless } = configs.reduce(
    (r, config) => {
      if (config.type === BuildTypes.BUNDLE) {
        config.alias = { ...aliasFromPaths.bundle, ...config.alias };
        r.bundle.push(config);
      } else if (config.type === BuildTypes.BUNDLESS) {
